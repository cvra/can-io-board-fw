...
void mpu60X0_setup(mpu60X0_t *dev, int config)
{
    ...
    if (dev->spi) { // disable I2C interface
        ...
        // bypass MPU auxiliary I2C
        uint8_t reg = mpu60X0_reg_read(dev, MPU60X0_RA_INT_PIN_CFG);
        chThdSleepMilliseconds(1);
        mpu60X0_reg_write(dev, MPU60X0_RA_INT_PIN_CFG, reg | MPU60X0_INTCFG_I2C_BYPASS_EN);
        chThdSleepMilliseconds(1);
    }
    // gyro full scale
